...
本功能的令牌表和版本审计字段直接合并在 `0001_initial.sql`。如果本机曾运行过旧版 EasyNote，请先导出需要保留的测试笔记，停止开发服务，删除 `.wrangler/state/v3/d1/` 后重新运行 `bash dev.sh`。项目尚未部署，因此不提供旧数据库增量迁移。
...
| 工具 | 权限 | 用途 |
| --- | --- | --- |
| `easynote_search_notes` | 只读 | 搜索标题和正文，可按视图、标签分页 |
...
```text
检索 EasyNote 时先调用 easynote_search_notes，再通过 easynote_read_note 读取所需正文。
...
- 搜索接口使用参数化 SQL `LIKE`，支持中文子串，单页最多 50 篇。
...
| `GET /api/integrations/notes` | 搜索和分页列出笔记 |
...
[`test/ai-evaluation.xml`](../test/ai-evaluation.xml) 包含 10 个只读问题。`npm run test:ai` 会把测试数据写入隔离运行时，并通过真实 MCP stdio 客户端检查搜索、分段读取、权限隔离和 revision 冲突。
