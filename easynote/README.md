...
一个面向个人使用的自托管 Markdown 笔记应用。React + TypeScript 前端，Cloudflare Worker API，D1 保存笔记，私有 R2 保存图片与附件。

## 已实现

- 单个初始化账号，用户名密码登录、修改密码、全端登出、交互式密码恢复、HttpOnly 会话 Cookie、CSRF 与来源检查、登录限流。
...
| `bash reset-password.sh --local` | 重置本地账号密码并撤销全部会话和 AI 令牌 |
| `bash reset-password.sh --remote` | 重置生产账号密码并更新初始化验证器 |
...
Service Worker 只预缓存应用外壳，不缓存 `/api`、登录会话、笔记正文或私有文件。用户可在“设置 → 离线笔记库”显式启用按账号隔离的 IndexedDB 镜像；启用后会增量保存全部笔记和引用文件，支持离线冷启动、全文搜索、阅读与编辑。断网修改进入待同步队列，恢复有效会话后按原 revision 和 operationId 自动提交；冲突仍进入显式冲突处理。
...
设置中的“账户安全”支持验证当前密码后修改密码；成功后保留当前会话并撤销其他浏览器会话。“登出所有设备”会连当前会话一起撤销。忘记密码时使用 `reset-password.sh`，脚本要求显式选择本地或生产、隐藏输入并进行文字确认，同时撤销所有浏览器会话和 AI 令牌。
...
单篇笔记可通过工具栏按钮或 `Cmd/Ctrl+P` 导出 PDF。应用会使用当前标题、更新时间、标签和渲染后的正文生成浅色 A4 版本，并等待私有图片与 Mermaid 图表完成加载。桌面端使用系统打印对话框；移动端直接生成 PDF 文件，iOS 和 Android 优先打开系统保存/分享面板，不支持文件分享时回退为浏览器下载。
...
- 导入先检查清单、尺寸、路径、重复条目、文件 SHA-256 及引用完整性，再按正文全文查重，仅上传非重复笔记实际引用的文件。
...
测试覆盖账号改密与全端登出、离线冷启动和回传、AI 令牌、真实 MCP、幂等与并发、搜索、批量标签、双链、删除墓碑、历史、私有文件、灾备校验、ZIP 恢复、移动布局和编辑锁。

## 当前边界

- 离线镜像不加密，安全边界与浏览器配置文件和设备账号一致；敏感设备应启用系统磁盘加密。
- 不内置生成式 AI、协同编辑、图谱、公开分享、自动定时远程备份、用户管理或注册。AI 只能通过用户主动创建的受限令牌和本地 MCP 桥接器访问笔记。
