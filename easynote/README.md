...
- 笔记新建、独立标题、Markdown 编辑/预览、自动保存、置顶、归档、标签和中文关键词搜索。
- 同一账号最多保留一篇完全空白的正常笔记；再次新建时直接打开已有空白笔记。
- CodeMirror 6 编辑器，markdown-it 解析和 DOMPurify 清理，支持安全 HTML block。
...
- 可安装 PWA，提供独立窗口、桌面/主屏幕图标和应用外壳离线缓存。
- AI 读写分离接入：受限令牌和 MCP 搜索、读取、写入工具；AI 修改进入正常版本历史。
...
Service Worker 只预缓存当前构建的应用外壳、基础编辑器、HTML、CSS、Manifest 和图标；Mermaid 图表引擎首次预览时按需加载，不进入首装预缓存。`/api`、登录会话、笔记正文及私有图片不进入 Cache Storage，避免旧响应覆盖同步状态或在退出后残留账号数据。已打开页面断网后仍可写入 IndexedDB 草稿；离线冷启动只能打开应用外壳，读取云端笔记和图片仍需联网。
...
每次保存提交 `revision` 和随机 `operationId`。服务端在 D1 事务中校验旧修订号、更新正文、记录新版本并维护图片引用。标题、正文、标签、置顶、归档和回收站状态均未变化时按保存成功返回，但不递增修订号或生成历史版本；历史列表也会折叠旧版本遗留的连续重复记录。只有写入成功才能显示“已保存到云端”。
...
ZIP 包含 `manifest.json`、`notes/*.md` 和所引用的 `images/*`。Markdown 图片引用转换为相对路径，解压后可用普通 Markdown 阅读器阅读。笔记文件优先使用标题命名；无标题时使用 `未命名.md`，重名时自动追加序号，稳定 ID 仅保留在清单中。
...
- 导入先检查清单、尺寸、路径、重复条目、图片 SHA-256 及引用完整性，再按正文全文查重，仅上传非重复笔记实际引用的图片。
- 非重复笔记会创建新 ID 并重写图片引用，不覆盖现有内容；正文完全一致或完全空白的笔记会跳过，并在结果中显示数量。
...
| `GET /api/integrations/notes` | 使用 Bearer 令牌搜索和分页列出笔记 |
...
测试覆盖登录保护、账号隔离、AI 令牌权限与撤销、真实 MCP 工具调用、幂等与并发、中文搜索、删除墓碑、历史保留、图片鉴权与清理、刷新恢复草稿、冲突副本、ZIP 恢复、移动布局和同浏览器编辑锁。

## 当前边界

- 不是完整离线笔记库：PWA 只缓存应用外壳，不缓存账号 API、全库正文或私有图片；已打开页面断网后仍可写本地草稿，重新联网后手动补传。
- 不内置生成式 AI、协同编辑、双链、图谱、公开分享、自动远程备份、用户管理、注册或密码重置。AI 只能通过用户主动创建的受限令牌和本地 MCP 桥接器访问笔记。
