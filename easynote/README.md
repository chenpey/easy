...
Service Worker 只预缓存当前构建的 HTML、JavaScript、CSS、Manifest 和图标。`/api`、登录会话、笔记正文及私有图片不进入 Cache Storage，避免旧响应覆盖同步状态或在退出后残留账号数据。已打开页面断网后仍可写入 IndexedDB 草稿；离线冷启动只能打开应用外壳，读取云端笔记和图片仍需联网。
...
每次保存提交 `revision` 和随机 `operationId`。服务端在 D1 事务中校验旧修订号、更新正文、记录新版本并维护图片引用。标题、正文、标签、置顶和回收站状态均未变化时按保存成功返回，但不递增修订号或生成历史版本；历史列表也会折叠旧版本遗留的连续重复记录。只有写入成功才能显示“已保存到云端”。
