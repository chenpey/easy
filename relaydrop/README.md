...
- 每个文件显示总进度、当前校验/上传分片及“正在合并”“已上传”或“失败”状态。传输达到 100% 不等于发布完成，只有 R2 合并和 D1 发布都确认成功才显示“已上传”。
...
- 文件链接和二维码只包含站点内的受保护下载地址，不包含共享密码或会话凭据；新设备打开后仍需先登录。
...
| 上传达到 100% 但仍显示正在合并 | 所有分片已传输，但 R2 合并和 D1 发布尚未确认；等待最终结果，失败后继续上传会复用操作键 |
