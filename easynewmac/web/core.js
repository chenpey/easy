    ...
    if (shouldInstall) {
      ...
      if (nvmNodeItems.length > 0) {
        ...
        lines.push(
          ...
          "  local nvm_prefix nvm_script nvm_status profile existing_nvm_dir needs_nvm_dir needs_nvm_source",
          '  profile="$HOME/.zshrc"',
          '  /usr/bin/touch "$profile" || return 1',
          '  existing_nvm_dir="$(/usr/bin/grep -Em1 \'^[[:space:]]*(export[[:space:]]+)?NVM_DIR=\' "$profile" 2>/dev/null || true)"',
          '  if [[ -n "$existing_nvm_dir" && "$existing_nvm_dir" != \'export NVM_DIR="$HOME/.nvm"\' ]]; then',
          '    print -u2 -- "检测到自定义 NVM_DIR，已停止 Node.js 安装：$existing_nvm_dir"',
          "    return 1",
          "  fi",
          ...
          "  needs_nvm_dir=0",
          "  needs_nvm_source=0",
          '  [[ -n "$existing_nvm_dir" ]] || needs_nvm_dir=1',
          '  /usr/bin/grep -Fq "$nvm_script" "$profile" 2>/dev/null || needs_nvm_source=1',
          "  if (( needs_nvm_dir || needs_nvm_source )); then",
          '    if [[ -s "$profile" && -n "$(/usr/bin/tail -c 1 "$profile")" ]]; then',
          '      printf \'\\n\' >> "$profile" || return 1',
          "    fi",
          '    (( needs_nvm_dir == 0 )) || print -r -- \'export NVM_DIR="$HOME/.nvm"\' >> "$profile" || return 1',
          '    (( needs_nvm_source == 0 )) || printf \'[ -s "%s" ] && \\\\. "%s"\\n\' "$nvm_script" "$nvm_script" >> "$profile" || return 1',
          "  fi",
          "}",
          "",
      ...
      if (appStoreItems.length > 0) {
        lines.push(
          ...
          ...appStoreItems.map(
            (item) =>
              `  install_mas_app ${shellQuote(item.name)} ${shellQuote(item.installId)} || install_status=1`,
          ),
          "else",
          "  install_status=1",
