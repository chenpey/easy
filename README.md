...
基于 Cloudflare Workers 的多用户自托管 Markdown 图片笔记应用。用户即租户，笔记、附件、离线缓存和 AI 令牌完全隔离；管理员可管理用户及审批注册。提供自动保存、任务中心、PWA Share Target 快速收集、Obsidian/Markdown 导入、带逐页预览的跨平台 PDF 导出、可撤销限时只读分享、私有图片、搜索、回收站、版本历史、默认开启的完整离线笔记库和并发冲突保护，并适配桌面与移动端。
