...
点击“选择文件”可一次选择多个文件并立即开始上传。默认同时处理 3 个文件；每个非空文件再使用 R2 Multipart Upload 并发上传分片。文件级和分片级并发都受服务端配置控制。
...
- 通过上一页、下一页按游标翻页，每页默认 10 条，不一次拉取全部历史。记录数量显示当前页条数，有下一页时带 `+`，不是全库总数。
...
git add .
git commit -m "发布：EasyDrop v1.1.4 优化上传展示与历史分页"
```
