...
适合个人跨设备传递文本和文件，或少量可信成员共用一个分享空间。管理员可以在部署后新增、修改、禁用和删除用户。
...
- 不提供公开注册页面。首次部署交互创建一个管理员；管理员登录后通过页面顶部的“用户管理”新增用户、修改资料或密码、启用、禁用和删除用户，并可设置管理员或普通用户角色。
- 用户名统一转为小写，长度为 3～32 位，只允许字母、数字、点、下划线和连字符。
- 密码长度必须为 12～32 位，并同时包含数字、大写字母和小写字母。
- 每个浏览器登录后建立独立的服务端会话，默认空闲有效期为 30 天；每次认证请求都会将服务端会话和 Cookie 延长 30 天。
...
- 文件链接和二维码只包含站点内的受保护下载地址，不包含用户名、密码或会话凭据；新设备打开后会先进入登录页，登录成功自动开始下载并返回主页。
...
- 下载地址以内部文件 ID 定位对象，不暴露 R2 公开链接，也不在 URL 中携带用户名或密码。
...
| 表 | 保存内容 |
| --- | --- |
| `users` | 用户名、密码验证器、角色、启用状态和鉴权版本 |
| `items` | 文本或文件记录、受支持的图片媒体类型，以及 `pending`、`ready`、`deleting` 状态 |
...
| 每次历史版本轮询 | 1 次 Worker 调用、1 次 D1 会话和版本联合查询、1 行会话续期写入，不访问 R2 |
| 拉取历史分页 | Worker 调用及 D1 会话、历史列表、版本查询和会话续期写入；内容更新后可能追加此请求 |
...
D1 按扫描/写入的**行数**计费，不是 SQL 请求数。每个受保护请求都会为 30 天滑动会话写入一次新到期时间，因此下方轮询调用数也近似等于会话续期的最低写入行数。索引维护、历史版本递增、操作记录和到期删除还会增加写入，不能把“一次上传”直接当作“一行写入”。以 D1 Metrics 的 Rows Read / Rows Written 为准。
...
已部署实例需要修改 D1 和 R2 名称时，不能直接编辑 `wrangler.deploy.json`。Cloudflare 当前不支持修改 D1 数据库名或 R2 桶名，因此两者都需要创建目标资源、迁移并校验数据，再切换 Worker 绑定。
...
2. 创建目标 R2 桶、先为源 D1 应用待执行迁移，再把 Worker 切换到只读迁移模式；浏览和下载继续可用，登录、上传、删除等写操作暂时返回 `503`。
3. 拒绝存在未完成上传的迁移；没有未完成上传时，按 D1 中的有效文件清单逐个下载、复制，并对目标对象重新下载执行大小和 SHA-256 校验。
4. 导出源 D1，新建目标 D1 后导入全部结构和数据，核对文件清单，再将 Worker 同时切换到目标 D1/R2 并原子更新本地部署配置。
5. 任一步骤失败时尝试恢复源 D1/R2 的正常访问。迁移状态保存在忽略提交的 `wrangler.storage-migration.json`，修复原因后可重跑同一命令。

脚本不会自动删除旧 D1 或旧 R2。上线后逐个抽查用户、历史文件和临时链接，确认目标资源数据完整，再从 Cloudflare Dashboard 手工删除旧数据库和旧桶。迁移完成后的普通更新仍使用 `bash deploy.sh`。
...
| 参数 | 默认值 | 可配置范围 | 含义 |
| --- | --- | --- | --- |
| `SESSION_TTL_SECONDS` | `2592000` | 3600～31536000 秒 | 会话滑动有效期，默认 30 天；每次认证请求重新计时 |
...
`SESSION_TTL_SECONDS` 同时控制服务端会话和 Cookie 的滑动窗口。页面从会话接口读取上传、分片并发、文本和轮询配置，修改后应重新打开页面。`UPLOAD_CHUNK_BYTES` 不能低于 R2 Multipart 要求的 5 MiB；增加分片大小会提高单个 Worker 和浏览器请求的内存压力。上传上限还受 Cloudflare 套餐的实际请求体限制约束，应用允许配置不代表平台必然允许。
...
这是访问控制方案，不是端到端加密。Cloudflare 和拥有相应账户管理权限的人仍可能读取存储数据。已授权成员可以复制、下载、删除他人分享的内容，应只为可信成员创建账户。
...
| `GET` / `HEAD` | `/shared/<token>` | 无需登录 | 在令牌有效期内下载文件，支持单段 Range |
...
- 只有一个共享空间；支持管理员和普通用户，但没有用户独立目录、只读角色或逐文件授权。
...
`npm test` 使用真实 workerd、D1 和 R2 本地模拟运行 API 集成验收，同时用本地 HTTP 服务器验证 Cloudflare 协议调用、部署复用、失败恢复和权限错误，不使用生产凭据或数据。`test:ui` 检查桌面/手机流程、临时文件链接、用户管理、响应丢失重试、分片并发、暂停后刷新恢复、并发编辑和分页保持。
