...
- 首次部署交互创建一个管理员。管理员登录后可新增、修改、启用、禁用和删除其他用户，并可设置管理员或普通用户角色。
...
页面顶部可打开当前站点地址的二维码，也可复制访问地址。二维码只包含站点 Origin（协议、域名及必要的端口），不包含用户名、密码、Cookie 或其他登录凭据。
...
首次运行：隐藏输入 Token，自动发现账号（多账号时再选择），输入 Worker 名称和可选自定义域名，设置初始管理员用户名和密码，确认部署。数据库和存储桶名称由 Worker 名称生成，不必手动填写。
...
`npm test` 使用真实 workerd、D1 和 R2 本地模拟运行 API 集成验收，同时用本地 HTTP 服务器验证 Cloudflare 协议调用、部署复用、失败恢复和权限错误，不使用生产凭据或数据。`test:ui` 检查桌面/手机流程、用户管理、响应丢失重试、分片并发、暂停后刷新恢复、并发编辑和分页保持。
