...
- JPEG、PNG、GIF、WebP、AVIF 和 BMP 文件由浏览器在上传前生成最长边 256 像素、最大 256 KiB 的 WebP 缩略图；点击缩略图仍按附件下载原文件。SVG 不内联展示。
...
| 成功上传一个文件 | 初始化、每个分片和合并分别产生 Worker/D1 操作；R2 通常产生 `分片数 + 2` 次 A 类操作和 1 次 B 类对象检查，图片缩略图再增加 1 次 Worker 和 R2 A 类写入 |
...
Cron 按 30 天计算，即 `30 × 24 × 4`，即使无人使用也会运行。默认 5 MiB 分片时，200 MiB 文件通常包含 40 个分片，对 R2 约产生 42 次 A 类操作（创建、40 次分片、合并）；可生成缩略图的图片再增加 1 次 A 类写入。分片重传会继续增加 A 类操作。失败后的补偿清理、下载器的多个 Range/HEAD 请求也会增加计量；已由服务端确认且校验一致的分片不会重复上传。私有下载不使用公共缓存，因此每个有效下载请求都会读 R2。
...
| `PUT` | `/api/uploads/<id>/preview` | 写校验 | 为所属图片上传 1～262144 字节的 WebP 缩略图 |
